...
- **书的页面宽度必须固定，不能随「这一页有没有目录」变化。** 书页面固定三栏（`detail--with-book` 与 `detail--with-toc` 一起加），即使当前页标题太少、右侧没有目录也把那一列留着。否则从有目录的章翻到没目录的章，整页宽度会从 1136px 掉到 1016px，跳得很明显。`tests/e2e/book.spec.ts` 里有「书的各个页面宽度完全一致」和「项目页宽度约为博客正文页的 1.1 倍」两条用例守着。
